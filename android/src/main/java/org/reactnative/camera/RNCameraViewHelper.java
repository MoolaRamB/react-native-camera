...
public class RNCameraViewHelper {
  ...
  private static boolean isScanningStopped = false;
  ...
  public static void emitCameraReadyEvent(final ViewGroup view) {
    ...
    reactContext.runOnNativeModulesQueueThread(new Runnable() {
      @Override
      public void run() {
        // Ensure the UIManagerModule is available
        UIManagerModule uiManagerModule = reactContext.getNativeModule(UIManagerModule.class);
        if (uiManagerModule == null) {
          // Log error or handle the unavailable module case
          return;
        }
  ...
  // public static void emitBarcodesDetectedEvent(final ViewGroup view, final
  // WritableArray barcodes,
  // final byte[] compressedImage) {

  // // Log the barcodes data for debugging purposes
  // Log.d("RNCamera", "Barcodes data: " + barcodes.toString()); // Log the
  // barcodes content

  // // Get the ReactContext
  // final ReactContext reactContext = (ReactContext) view.getContext();

  // // Check if the ReactContext is initialized and active in bridgeless mode
  // if (reactContext.hasActiveCatalystInstance()) {
  // // Send event to JS side via the DeviceEventManager
  // reactContext
  // .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
  // .emit("onBarcodesDetected", barcodes); // "onBarcodesDetected" is the event
  // name on JS side
  // } else {
  // Log.e("RNCamera", "React Context is not active, cannot send event.");
  // }
  // }
  ...
  // final byte[] compressedImage) {
  // // Log the barcodes data for debugging purposes
  // Log.d("RNCamera", "Barcodes data: " + barcodes.toString()); // Ensure this is
  // showing the correct barcode data
  ...
  // @Override
  // public void run() {
  // // Ensure event is dispatched only if the ReactContext is active
  // if (reactContext.hasActiveCatalystInstance()) {
  // // Emit the event to JavaScript using DeviceEventEmitter
  // reactContext
  // .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
  // .emit("onBarcodesDetected", barcodes); // Send barcode data to JS
  // } else {
  // Log.e("RNCamera", "React Context is not active, cannot send event.");
  // }
  // }
  ...
  public static void emitBarcodesDetectedEvent(final ViewGroup view, final WritableArray barcodes,
      final byte[] compressedImage) {
    // Log the barcodes data for debugging purposes
    Log.d("RNCamera", "Barcodes data: " + barcodes.toString()); // Ensure this is showing the correct barcode data

    // Check if barcode data is not empty
    if (barcodes != null && barcodes.size() > 0) {
      final ReactContext reactContext = (ReactContext) view.getContext();

      // Check if ReactContext is active
      if (reactContext.hasActiveCatalystInstance()) {
        // Emit the barcode data to JavaScript once
        reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
            .emit("onBarcodesDetected", barcodes); // Send barcode data to JS

        // Log to confirm the event was sent
        Log.d("RNCamera", "Barcodes event emitted to JS.");
        stopBarcodeScanning(view);
        // Optionally, return or stop further processing after the first emission
        return; // Stop processing once the event has been sent successfully
      } else {
        Log.e("RNCamera", "React Context is not active, cannot send event.");
      }
    } else {
      // If barcodes are empty or null, log a warning or handle as needed
      Log.w("RNCamera", "No barcodes detected or barcodes array is empty.");
    }
  }

  private static void stopBarcodeScanning(ViewGroup view) {
    // Set the flag to stop further barcode scanning
    isScanningStopped = true;

    // You can also stop other processes or listeners related to barcode scanning
    // here.
    RNCameraView cameraView = (RNCameraView) view; // Assuming the view is RNCameraView
    cameraView.setShouldScanBarCodes(false); // Set the flag to disable barcode scanning
  }

  // send only once

  // Log.d("RNCamera", "Barcodes detected: " + barcodes.toString());
  // public static void emitBarcodesDetectedEvent(final ViewGroup view, final
  // WritableArray barcodes,
  // final byte[] compressedImage) {
  ...
  // if (reactContext == null) {
  // Log.e("RNCamera", "ReactContext is null!");
  // return; // Don't continue if the context is null
  // }
  ...
  // if (uiManagerModule == null) {
  // Log.e("RNCamera", "UIManagerModule is null!");
  // return; // Don't continue if UIManagerModule is null
  // }

  // EventDispatcher eventDispatcher = uiManagerModule.getEventDispatcher();
  // if (eventDispatcher == null) {
  // Log.e("RNCamera", "EventDispatcher is null!");
  // return; // Don't continue if EventDispatcher is null
  // }

  // BarcodesDetectedEvent event = BarcodesDetectedEvent.obtain(view.getId(),
  // barcodes, compressedImage);

  // // Log the event before dispatching for debugging
  // Log.d("RNCamera", "Dispatching BarcodesDetectedEvent: " + event.toString());

  // eventDispatcher.dispatchEvent(event); // Dispatch the event
  // }
  // });
  // }
  ...
  // public static void emitBarCodeReadEvent(final ViewGroup view, final Result
  // barCode, final int width, final int height,
  // final byte[] compressedImage) {
  // final ReactContext reactContext = (ReactContext) view.getContext();
  // reactContext.runOnNativeModulesQueueThread(new Runnable() {
  // @Override
  ...
  public static void emitBarCodeReadEvent(final ViewGroup view, final Result barCode, final int width, final int height,
    ...
    reactContext.runOnNativeModulesQueueThread(new Runnable() {
      @Override
      public void run() {
        if (barCode == null || compressedImage == null) {
          // Handle the error, log, or skip
